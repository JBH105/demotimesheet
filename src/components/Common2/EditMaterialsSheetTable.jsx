...
function EditMaterialsTimesheet({ selectedDateRange, material, items, timesheetDataFromFirebase, fetch, setFetch, type }) {
  ...
  const dbName = material.endsWith("s") ? material.toLowerCase() : material.toLowerCase() + "s";

  useEffect(() => {
    if (selectedDateRange.length === 2) {
      generateTimesheetData(selectedDateRange, items);
    }
  }, [selectedDateRange, items, timesheetDataFromFirebase]);

  const generateTimesheetData = (selectedDateRange, items) => {
    ...
    items.forEach(item => {
      ...
      const existingTimesheetData = timesheetDataFromFirebase.find(data => data[material.toLowerCase()] === item);

      if (existingTimesheetData) {
        // Use the data from prop
        timesheetObject[material.toLowerCase()] = existingTimesheetData[material.toLowerCase()];
  ...
  const handleSaveToFirebase = async () => {
    ...
    try {
      ...
      for (const record of timesheetData) {
        const uniqueIdentifier = `${record[material.toLowerCase()]}-${record.type}-${record.period}`;
        const updatedRecord = { ...record, status: 'editing' };
        const docRef = await setDoc(doc(db, dbName, uniqueIdentifier), updatedRecord);
      }
...
export default EditMaterialsTimesheet;
